...
class KLIFFGraphNetwork(GraphModuleMixin, torch.nn.Sequential):
    ...
    # Copied from https://pytorch.org/docs/stable/_modules/torch/nn/modules/container.html#Sequential
    # with type annotations added
    def forward(self, x, pos, edge_index0, edge_index1, edge_index2, contributing):
    # def forward(self, input):
        # pos = input[AtomicDataDict.POSITIONS_KEY]
        # edge_index = input[AtomicDataDict.EDGE_INDEX_KEY]
        # x = input[AtomicDataDict.ATOM_TYPE_KEY].squeeze(-1)
        # contributing = torch.zeros(64, dtype=torch.long)
